...
function buildOAuthClient(redirectUri) {
    return new googleapis_1.google.auth.OAuth2(CLIENT_ID, CLIENT_SECRET, redirectUri || REDIRECT_URI);
}
function expiryToISOString(expiry) {
 ...
 */
function parseBookingStartDateTime(bookingDate, bookingTimeRaw, tz) {
    if (!bookingDate || !bookingTimeRaw)
        return null;
    ...
    if (s.includes("T") || /Z$/.test(s) || /\+\d{2}:\d{2}/.test(s)) {
        const dtIso = luxon_1.DateTime.fromISO(s, { zone: tz });
        if (dtIso.isValid)
            return dtIso;
    }
    ...
    if (m) {
        ...
        if (dateParts.length >= 3 && dateParts.every((n) => !Number.isNaN(n))) {
            ...
            const dt = luxon_1.DateTime.fromObject({ year, month, day, hour, minute, second, millisecond: 0 }, { zone: tz });
            if (dt.isValid)
                return dt;
        }
        else {
            const isoAttempt = luxon_1.DateTime.fromISO(`${bookingDate}T${s}`, { zone: tz });
    ...
    for (const fmt of ampmFormats) {
        const dt = luxon_1.DateTime.fromFormat(s, fmt, { zone: tz });
        if (dt.isValid) {
            const dateParts = bookingDate.split("-").map((p) => Number(p));
            if (dateParts.length >= 3 && dateParts.every((n) => !Number.isNaN(n))) {
                const [year, month, day] = dateParts;
    ...
    try {
        ...
        if (!isNaN(naive.getTime())) {
            const dt = luxon_1.DateTime.fromJSDate(naive, { zone: tz });
            if (dt.isValid)
                return dt;
        }
    }
...
const GoogleCalendarService = {
    generateAuthUrl: async (counselorId) => {
        const oauth2Client = buildOAuthClient();
        const state = crypto_1.default.randomBytes(16).toString("hex");
        await counselorRepository_1.default.setGoogleOAuthState(counselorId, state);
        // use the canonical scopes from authService
        const url = oauth2Client.generateAuthUrl({
            access_type: "offline",
            scope: authService_1.OAUTH_SCOPES,
            prompt: "consent",
            state,
        });
        return { url, state };
    ...
    exchangeCodeAndStoreTokens: async (args) => {
        ...
        if (state) {
            foundCounselor = await counselorRepository_1.default.findCounselorByOAuthState(state);
            if (!foundCounselor)
        ...
        const counselorRow = await counselorRepository_1.default.getCounselorById(cid);
        const calendarId = counselorRow?.email ?? null;
        const expiryIso = expiryToISOString(tokens.expiry_date ?? oauth2Client.credentials.expiry_date);
        await counselorRepository_1.default.storeGoogleTokens(cid, tokens.access_token ?? null, tokens.refresh_token ?? null, expiryIso, calendarId);
    ...
    getAuthorizedCalendarClient: async (counselorId) => {
        ...
        if (!counselor.google_refresh_token) {
            throw new GoogleAuthError("Counselor has not connected Google Calendar or refresh token missing", "NO_REFRESH_TOKEN");
        }
        const oauth2Client = buildOAuthClient();
        ...
        try {
            ...
            const newExpiryIso = expiryToISOString(cred.expiry_date ?? cred.expiry_date);
            if (newAccess || newExpiryIso) {
                await counselorRepository_1.default.storeGoogleTokens(counselor.counselor_id, newAccess, counselor.google_refresh_token ?? null, newExpiryIso, counselor.google_calendar_id ?? null);
            }
        }
        catch (e) {
            const msg = (e?.message && String(e.message)) || "Google token refresh failed";
            // detect typical invalid_grant
            if ((e?.response?.data &&
                String(e.response.data.error) === "invalid_grant") ||
                /invalid_grant/i.test(msg)) {
                throw new GoogleAuthError("Google refresh token invalid or revoked (invalid_grant). Reauthorization required.", "INVALID_GRANT", e);
            }
            // Other auth errors
     ...
     */
    getAvailableSlots: async (counselorId, dateStr, durationMinutes = 60) => {
        // getAuthorizedCalendarClient will throw GoogleAuthError on auth problems
        const { calendar, counselor } = await GoogleCalendarService.getAuthorizedCalendarClient(counselorId);
        const timezone = counselor.timezone ?? DEFAULT_TIMEZONE;
        const dayStart = luxon_1.DateTime.fromISO(dateStr, { zone: timezone })
            .startOf("day")
            .toISO();
        const dayEnd = luxon_1.DateTime.fromISO(dateStr, { zone: timezone })
            .endOf("day")
            .toISO();
        ...
        try {
            ...
            fbRes = await calendar.freebusy.query(freebusyReq);
        }
        catch (e) {
        ...
        const busyRanges = (busy || []).map((b) => {
            const s = luxon_1.DateTime.fromISO(b.start).setZone(timezone).toISO() ?? "";
    ...
    bookSession: async (payload) => {
        ...
        const event = {
            summary: summary ?? `Counselling session with ${student_email ?? "student"}`,
        ...
        if (student_email)
            event.attendees.push({ email: student_email });
        ...
        try {
            if (typeof student_id !== "undefined" && student_id !== null) {
                ...
                const found = await studentRepository_1.studentRepository.findByEmail(student_email);
                if (found && found.student_id) {
                    finalStudentId = found.student_id;
                }
                else {
                    const guestName = (student_email && student_email.split("@")[0]) || "Guest Student";
                    const createdStudent = await studentRepository_1.studentRepository.create(guestName, student_email);
            ...
            throw new Error("studentRepository failure: " + err.message);
        }
        // Create booking row in DB
        const bookingRow = await bookingRepository_1.default.createBooking(finalStudentId, counselor_id, booking_date, startDT.toFormat("HH:mm:ss"), year_level ?? null, additional_notes ?? null);
        // Try to store google event id (non-fatal)
        try {
            if (bookingRow &&
                googleEvent?.id &&
                typeof bookingRepository_1.default.updateGoogleEventId === "function") {
                await bookingRepository_1.default.updateGoogleEventId(bookingRow.booking_id, googleEvent.id);
            }
        }
        catch (e) {
        ...
        try {
            ...
            if (typeof bookingIdOrRow === "object" && bookingIdOrRow !== null) {
                ...
                if (typeof bookingRepository_1.default.getBookingById === "function") {
                    bookingRow = await bookingRepository_1.default.getBookingById(bookingIdOrRow);
                }
                else {
                    throw new Error("bookingRepository.getBookingById not available to fetch booking row");
                }
            }
            if (!bookingRow) {
                return { success: false, reason: "booking_not_found" };
            }
            const counselorId = Number(bookingRow.counselor_id ?? bookingRow.therapistId) || null;
            if (!counselorId) {
            ...
            const calId = counselor.google_calendar_id ?? counselor.email;
            // Parse new start/end
            const newStartDT = parseBookingStartDateTime(booking_date, booking_time, tz);
            ...
            let eventId = bookingRow.google_event_id ??
            ...
            if (!eventId) {
                ...
                const q = bookingRow.student_email ?? String(bookingRow.booking_id ?? "");
            ...
            if (!eventId) {
                ...
                const event = {
                    summary: `Counselling session with ${bookingRow.student_email ?? "student"}`,
                ...
                try {
                    if (createdEvent?.id &&
                        typeof bookingRepository_1.default.updateGoogleEventId === "function") {
                        await bookingRepository_1.default.updateGoogleEventId(bookingRow.booking_id, createdEvent.id);
                    }
                }
                catch (e) {
                    console.warn("Could not persist new google event id to booking row:", e);
                }
                return {
                    success: true,
            ...
            try {
                ...
                try {
                    ...
                    try {
                        if (createdEvent2?.id &&
                            typeof bookingRepository_1.default.updateGoogleEventId === "function") {
                            await bookingRepository_1.default.updateGoogleEventId(bookingRow.booking_id, createdEvent2.id);
                        }
                    }
                    catch (ee) {
    ...
    deleteEvent: async (counselorId, googleEventId) => {
        try {
            const { calendar, counselor } = await GoogleCalendarService.getAuthorizedCalendarClient(counselorId);
